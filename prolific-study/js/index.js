...
var definedPositions = definedPositions || [];
...
/*Blockly.Blocks['custom_trigger'].customContextMenu = function(options) {
  if (this.isInFlyout) {
    ...
    //options.push(deleteOption);
    var copyOption = {enabled: true};
    var name = this.getFieldValue('TRIGGER');
    copyOption.text = "Clone trigger '%1'".replace('%1', name);
    var block = this;
    var newName = getCollisionFreeTriggerName(name);
    var xmlCopy = Blockly.utils.xml.createElement('block');
    xmlCopy.setAttribute('type', 'custom_trigger');
    var trigger = Blockly.utils.xml.createElement('field');
    trigger.setAttribute('name', 'TRIGGER');
    trigger.innerHTML = newName;
    xmlCopy.appendChild(trigger);
    ...
    copyOption.callback = function () {
      ...
      var copiedRightWorkspace = Blockly.inject(newRightDiv.id,
        { media: pathPrefix + 'blockly/media/',
          toolbox: document.getElementById("toolboxTrigger"),
          trashcan: true,
          move:{
            scrollbars: true,
            drag: false,
      ...
      Blockly.Xml.domToWorkspace(savedDom, copiedRightWorkspace);
      newRightDiv.style.display = 'none';
      copiedRightWorkspace.getAllBlocks().find(block => block.type == 'custom_triggerheader').getField('TRIGGER').setValue(newName);
      rightWorkspaces['!' + newName] = copiedRightWorkspace;
      copiedRightWorkspace.addChangeListener(logEvent);
      redrawStack();
      //Blockly.ContextMenu.callbackFactory(block, xmlCopy)();
...
leftWorkspace.registerToolboxCategoryCallback(
  'TRIGGERS', flyoutTriggerCategory);
leftWorkspace.registerToolboxCategoryCallback(
  'LOCATIONS', flyoutLocationCategory);
leftWorkspace.getAllBlocks().map(b => b.setDeletable(false));
