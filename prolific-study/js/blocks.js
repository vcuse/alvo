...
if (typeof taskMachineStations == "undefined") {
...
Blockly.defineBlocksWithJsonArray([
    ...
    {
        "type": "custom_triggerheader",
        "message0": "%1 when all of",
        "args0": [
            {
              "type": "field_input",
              "name": "TRIGGER",
              "text": "Something is triggered",
              "spellcheck": false
            }
        ],
        "inputsInline": false,
    ...
    {
        "type": "custom_machineidle",
        "message0": "Machine at %1 is %2",
        "args0": [
            {
                "type": "field_dropdown",
                "name": "SITE",
    ...
    {
        "type": "custom_emptygripper",
        "message0": "Robot gripper is %1",
        "args0": [
            {
                "type": "field_dropdown",
                "name": "EMPTYSTATUS",
...
function getCollisionFreeTriggerName(name) {
  var counter = 2;
  var collisions = definedTriggers.filter(trigger => trigger == name);
  var newName = name;

  while (collisions.length > 0) {
    collisions = definedTriggers.filter(trigger => trigger == name + counter);  
    newName = name + counter;
    counter += 1;
  }
  return newName;
...
var flyoutTriggerCategory = function(workspace) {
  var xmlList = [];
  var newBlock = Blockly.utils.xml.createElement('block');
  newBlock.setAttribute('type', 'custom_trigger');
  newBlock.setAttribute('gap', 30);
  var nameField = Blockly.utils.xml.createElement('field');
  nameField.setAttribute('name', 'TRIGGER');
  var name = getCollisionFreeTriggerName("Something is triggered");
  nameField.appendChild(Blockly.utils.xml.createTextNode(name));
  newBlock.appendChild(nameField);
  xmlList.push(newBlock);

  var blockList = flyoutTriggerCategoryBlocks(workspace);
  xmlList = xmlList.concat(blockList);
  return xmlList;
};
...
var flyoutTriggerCategoryBlocks = function(workspace) {
  var xmlList = [];
  if (definedTriggers.length > 0) {
  var heading = Blockly.utils.xml.createElement('label');
    heading.setAttribute('text', 'Existing triggers:');
    heading.setAttribute('web-class','toolboxHeading')
    xmlList.push(heading);
  }
  var done = [];
  for (var i = 0, trigger; (trigger = definedTriggers[i]); i++) {
    if (done.includes(trigger)) 
      continue;
    done.push(trigger);
    // New variables are added to the end of the variableModelList.
    var block = Blockly.utils.xml.createElement('block');
    block.setAttribute('type', 'custom_trigger');
    block.appendChild(
    generateTriggerFieldDom(trigger));
    xmlList.push(block);
  }
  return xmlList;
};
...
var generateTriggerFieldDom = function(trigger) {
  var field = Blockly.utils.xml.createElement('field');
  field.setAttribute('name', 'TRIGGER');
  var name = Blockly.utils.xml.createTextNode(trigger);
  field.appendChild(name);
  return field;
};
